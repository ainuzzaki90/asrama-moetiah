...
const Api = (() => {
  ...
  return {
    ...
    uploadFile: (file) => {
      ...
      return gasPost({ action:"uploadFile", filename: file.name, mimeType: file.type, data: file.data })
        .then(r => {
          if(!r || r.ok === false) throw new Error(r?.message || "Gagal mengunggah file ke Google Drive");
          return { name: file.name, tipe: file.type, url: r.data.url };
        });
    },
