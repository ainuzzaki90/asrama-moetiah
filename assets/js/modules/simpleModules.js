  ...
  Router.register("kesehatan", async (container) => {
    ...
    CrudModule.mount(container, {
      ...
      formFields:[
        ...
        { name:"tindakan", label:"Tindakan/Penanganan", type:"textarea", col:12 },
        { name:"petugas", label:"Petugas Penanganan", col:12 },
      ],
      onBeforeSave: async (data) => {
        await Api.create("notifications", { tipe:"Kesehatan", judul:"Catatan kesehatan baru", pesan:`${Cache.siswaName(data.siswaId)} — ${data.keluhan}`, dibaca:false, waktu:new Date().toISOString() });
        Notif.refreshBadge();
        return data;
      }
    });
