...
const Reports = (() => {
  ...
  async function renderPage(container){
    ...
    let currentRows = [], currentDef = REPORT_TYPES[0];
    ...
    async function loadReport(){
      ...
      document.querySelector("#repTable thead").innerHTML = `<tr>${currentDef.cols.map(c=>`<th>${c}</th>`).join("")}</tr>`;
      document.querySelector("#repTable tbody").innerHTML = currentRows.map(row=>`<tr>${row.map(v=>`<td>${v}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${currentDef.cols.length}" class="text-center text-muted py-4">Tidak ada data pada filter ini</td></tr>`;
    }
