...
(function(){
  ...
  async function render(container){
          ...
          <div class="card-body-mbms">
            <div class="table-responsive">
              <table class="table table-hover align-middle" id="saldoTable" style="width:100%">
                <thead><tr><th>Siswa</th><th>Kelas</th><th>Kamar</th><th>Saldo Saat Ini</th><th>Aksi</th></tr></thead>
                <tbody></tbody>
              </table>
            </div>
          </div>
    ...
    async function reload(){
      ...
      </tr>`).join("") || `<tr><td colspan="5" class="text-center text-muted py-4">Belum ada data siswa</td></tr>`;

      document.querySelectorAll("#saldoTable [data-act]").forEach(btn=>{
        btn.onclick = () => openPassbook(btn.dataset.id, data);
      });
      dt = $("#saldoTable").DataTable({ pageLength:10, columnDefs:[{orderable:false,targets:4}], language:{search:"Cari siswa:",zeroRecords:"Data tidak ditemukan"} });
    }
    ...
    const html = `<div class="text-start">
      ...
      <div class="table-responsive" style="max-height:340px;overflow-y:auto">
        <table class="table table-sm">
          <thead><tr><th>Tanggal</th><th>Jenis</th><th>Keterangan</th><th class="text-end">Jumlah</th><th class="text-end">Saldo</th></tr></thead>
          <tbody>
            ${history.map(h=>`<tr>
            ...
            </tr>`).join("") || `<tr><td colspan="5" class="text-center text-muted py-3">Belum ada transaksi</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>`;
