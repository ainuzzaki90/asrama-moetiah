...
/**
 * Run this ONCE from the Apps Script editor after pasting this file into a
 * fresh Google Sheet. It creates every required tab with the correct header
 * row and seeds demo accounts + sample data so the system is usable
...
function setupDatabase(){
  ...
  // Remove the default "Sheet1" if it still exists and is empty
  const def = ss.getSheetByName("Sheet1");
  if(def && def.getLastRow() <= 1) ss.deleteSheet(def);

  seedDemoData(ss);
  SpreadsheetApp.getUi().alert("Setup selesai! Sheet, header, dan data demo berhasil dibuat. Silakan lanjutkan ke Deploy > New deployment.");
}
