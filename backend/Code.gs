...
const SHEET_NAMES = [
  ...
  "kesehatan","tumbuh_kembang","inventaris","tabungan","piket_putaran","kebersihan",
...
function doPost(e){
  try{
    ...
    if(action === "uploadFile") return jsonOut(handleUploadFile(body.filename, body.mimeType, body.data));
...
// Used for attachments such as medical certificates / X-ray scans in Kesehatan Siswa.
// Files are stored in a dedicated Drive folder (auto-created on first use) and set to
// "anyone with the link can view" so the URL alone (stored in the Sheet cell) is enough
// to display/download it — this avoids Google Sheets' ~50,000-character cell size limit
// that would break if the raw file were embedded as base64 directly in a cell.

const UPLOAD_FOLDER_NAME = "MBMS_Lampiran";
...
function handleUploadFile(filename, mimeType, base64DataUrl){
  try{
    const commaIdx = base64DataUrl.indexOf(",");
    const base64 = commaIdx > -1 ? base64DataUrl.substring(commaIdx + 1) : base64DataUrl;
    ...
    return { ok:true, data: { url: file.getUrl(), fileId: file.getId(), name: filename } };
  }catch(err){
    return { ok:false, message: "Gagal mengunggah file: " + String(err) };
  }
}
...
function setupDatabase(){
  ...
  const schemas = {
    ...
    kesehatan:    ["id","siswaId","tanggal","keluhan","tindakan","petugas","statusRujuk","lampiran"],
    ...
    piket_putaran: ["id","gedung","putaranKe","tanggalMulai","ketuaKamarIds","petugasHarian"],
...
const SEED_DATA = {
  ...
  piket_putaran: [
    ["PK-0001","Putra",1,"2026-07-18", JSON.stringify(["S-0001"]), JSON.stringify({
      Sabtu:["S-0002","S-0006"], Minggu:["S-0006","S-0002"], Senin:["S-0002","S-0006"],
      Selasa:["S-0006","S-0002"], Rabu:["S-0002","S-0006"], Kamis:["S-0006","S-0002"], Jumat:["S-0002","S-0006"],
    })],
  ],
};
